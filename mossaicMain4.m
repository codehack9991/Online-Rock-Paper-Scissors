...
%     
    ...
    global NUMROUNDS
    NUMROUNDS = 5;
    title('MOSSAIC By Team MemBRAIN');
%     global ALPHA
%     ALPHA = 0.001;
...
%     imtool(initSnap);
    ...
    while GAMEOVER == 0
        ...
        if numberOfPlayers == 1
            ...
            cnap = getsnapshot(vid);
            cnap = getsnapshot(vid);
                    pause(1.4);
                    

                    snap1 = imcrop(cnap, CROPBB1);
                    snapHand1 = seperateHand(snap1, RGBVALUES1);
% 
%                     snap2 = imcrop(snap, CROPBB2);
%                     snapHand2 = seperateHand(snap2, RGBVALUES2);

                    cimage1 = getFinalImage(snapHand1);
                    [rows, cols] = size(cimage1);
                    cimage2 = zeros(rows, cols);

                    cimage = [cimage1 cimage2];

                    cimage = addBoxes(cimage, 1);
                    
%                     cimage = showMessage(cimage,'Get your hand ready', [5,5]);
%                      cimage = showMessage(cimage, num2str(SCORES(1)), DOWNLOC1);
%                     cimage = showMessage(cimage, num2str(SCORES(2)), DOWNLOC2);
%                     
%                     
%         %             cimage = insertText(cimage,[0,0],'HELLO','FontSize',30, 'BoxOpacity',0, 'TextColor','white');
%                     imshow(cimage);
...
%                     snapHand2 = seperateHand(snap2, RGBVALUES2);
        ...
        %             b = now;
                ...
                if(player1 * player2 ~= 0)
                    ...
                    if winner ~= 0
                        ...
                        if winner == 1
                            [rows, cols] = size(p1Wins);
                            image = zeros(rows,cols,3);
                            image(:,:,1) = p1Wins;
                            image(:,:,2) = p1Wins;
                            image(:,:,3) = p1Wins;
                            image = showMessage(image, NAME(player1, :) ,P1LOC);
                            image = showMessage(image, NAME(player2, :) ,P2LOC);
                            imshow(image);
                            
                        else
                            [rows, cols] = size(p2Wins);
                            image = zeros(rows,cols,3);
                            image(:,:,1) = p2Wins;
                            image(:,:,2) = p2Wins;
                            image(:,:,3) = p2Wins;
        ...
        cnap = getsnapshot(vid);
                    ...
                    cnap1 = imcrop(cnap, CROPBB1);
                    snapHand1 = seperateHand(cnap1, RGBVALUES1);

                    cnap2 = imcrop(cnap, CROPBB2);
                    snapHand2 = seperateHand(cnap2, RGBVALUES2);

                    cimage1 = getFinalImage(snapHand1);
                    cimage2 = getFinalImage(snapHand2);

                    cimage = [cimage1 cimage2];

                    cimage = addBoxes(cimage, 2);
        %             image = insertText(image,[0,0],'HELLO','FontSize',30, 'BoxOpacity',0, 'TextColor','white');
                    cimage = showMessage(cimage, 'Get Your Hands Ready!',[5,5]);
                    cimage = showMessage(cimage, num2str(SCORES(1)), DOWNLOC1);
                    cimage = showMessage(cimage, num2str(SCORES(2)), DOWNLOC2);
                    cimage1 = imcrop(cimage1, WORKINGBB1);
                    cimage2 = imcrop(cimage2, WORKINGBB1);
%                     imshow(image);
                    pause(2)
                while(1)
                    pause(0.1)
...
%                     imshow(image);
                    ...
                    if(isStable(image1,cimage1)==1&&isStable(image2,cimage2)==1)
                        break;
